...
- [ ] Credential and data exposure considered. A slice that introduces a new credential or a new class of data access — a second database connection, an API token, a raw reading served over the protocol — is a scope change requiring its own PRD. As of "Coverage Values over Read-Only Postgres" (2026-08-06), this repo holds exactly one credential (`MCP_DATABASE_URL`, read-only, scoped to `biometric_readings`)
...
- [ ] Dev environment plan clear — Node 18+ per `package.json#engines`, `npm install` (which installs the husky hooks), `JERKAI_REPO` pointing at a local jerkai checkout if the slice touches the vendored files or needs jerkai's migrations, and `MCP_DATABASE_URL` if the slice touches `list_available_metrics`'s coverage query — all documented by name and purpose, never by value, in `.env.example`
...
- [ ] ~~Production migration plan~~ — **not applicable**: "Coverage Values over Read-Only Postgres" (2026-08-06) re-armed this by introducing a database connection (expected with 1b), and confirmed it stays not applicable — that slice reads jerkai's existing schema as-is and runs no migration of its own; still re-arms for whichever slice next needs one
